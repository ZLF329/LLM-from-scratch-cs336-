...
def run_multihead_self_attention(
    ...
    model.q_linear.weight.data.copy_(q_proj_weight)
    model.k_linear.weight.data.copy_(k_proj_weight)
    model.v_linear.weight.data.copy_(v_proj_weight)
    model.o_linear.weight.data.copy_(o_proj_weight)
    output = model(in_features)
    return output
...
def run_transformer_block(
    ...
    model.attn.o_linear.weight.data.copy_(weights["attn.output_proj.weight"])
...
def run_transformer_lm(
    ...
    for layer_idx in range(num_layers):
        model.layers[layer_idx].attn.q_linear.weight.data.copy_(weights[f"layers.{layer_idx}.attn.q_proj.weight"])
        model.layers[layer_idx].attn.k_linear.weight.data.copy_(weights[f"layers.{layer_idx}.attn.k_proj.weight"])
        model.layers[layer_idx].attn.v_linear.weight.data.copy_(weights[f"layers.{layer_idx}.attn.v_proj.weight"])
        model.layers[layer_idx].attn.o_linear.weight.data.copy_(weights[f"layers.{layer_idx}.attn.output_proj.weight"])
